...
def sql(request, values, output=False):
    connection = sqlite3.connect("DB")
    cursor = connection.cursor()
    cursor.execute("""CREATE TABLE IF NOT EXISTS database(id INTEGER, room_type TEXT, tv INTEGER, price INTEGER)""")

    if output and len(values) < 1:
        cursor.execute(request, values)
        data = cursor.fetchall()
        return data

    elif output and len(values) >= 1:
        data = []
        for j_id in values:
            cursor.execute(request, j_id)
...
while True:
    ...
    if choice == 1:
        ...
        user_values = []
        user_request = """INSERT INTO database(id, room_type, tv, price) VALUES(?, ?, ?, ?)"""

        for i_id in range(string_id + 1, string_id + amount + 1):
            room_type = input("\nТип комнаты: ")
            tv = int(input("Наличие телевизора в комнате: "))
            price = int(input("Цена комнаты: "))
            user_values.append((i_id, room_type, tv, price))

        string_id += amount
        sql(user_request, user_values)
        print("\nТаблица успешно пополнена")

    elif choice == 2:
        amount = int(input("\nКоличество удаленных строк: "))

        user_values = []
        user_request = """DELETE FROM database WHERE id = ?"""

        for _ in range(amount):
            i_id = int(input("id комнаты: "))
            user_values.append((i_id,))
        ...
        user_values = []
        user_request = """UPDATE database SET room_type = ?, tv = ?, price = ? WHERE id = ?"""

        for _ in range(amount):
            i_id = int(input("\nid изменяемой комнаты: "))
            room_type = input("Новый тип комнаты: ")
            tv = int(input("Новое наличие телевизора в комнате: "))
            price = int(input("Новая цена комнаты: "))
            user_values.append((room_type, tv, price, i_id))
        ...
        if option == 1:
            user_request = """SELECT * FROM database"""
            user_values = ()
            info = sql(user_request, user_values, output=True)

        else:
            amount = int(input("\nКоличество выводимых строк: "))

            user_values = []
            user_request = """SELECT * FROM database WHERE id = ?"""
